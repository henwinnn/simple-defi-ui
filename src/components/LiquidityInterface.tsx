...
const LiquidityInterface = () => {
  ...
  const getTokenRatio = () => {
    ...
    const campPerUsdc =
      Number(poolInfo.reserveA) /
      Math.pow(10, 18) /
      (Number(poolInfo.reserveB) / Math.pow(10, 6));
  ...
  useEffect(() => {
    if (
      ...
      const data = calculateAddLiquidity(
        amountA,
        amountB,
        TOKENS.CAMP,
        TOKENS.USDC
  ...
  const handleMaxA = () => {
    const balance =
      Number(campBalance.balance) / Math.pow(10, TOKENS.CAMP.decimals);
    handleAmountAChange(balance.toString());
  };

  const handleMaxB = () => {
    const balance =
      Number(usdcBalance.balance) / Math.pow(10, TOKENS.USDC.decimals);
  ...
  const handleRemoveLiquidity = async () => {
    const lpAmount =
      (Number(userPosition.lpTokenBalance) * removePercentage) /
      100 /
      Math.pow(10, 18);
  ...
  const isInsufficientBalance = () => {
    if (!amountA || !amountB || !campBalance.balance || !usdcBalance.balance)
      return false;
    ...
    return (
      campAmount > Number(campBalance.balance) ||
      usdcAmount > Number(usdcBalance.balance)
    );
  };
        ...
        {activeTab === "add" ? (
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            {ratio && (
              ...
              >
                ...
                >
                  ...
                  >
                    💡 Enter amount in either field - the other will
                    auto-calculate
                  </div>
                </div>
              </div>
            ...
            >
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <span
                  className="text-xs sm:text-sm truncate ml-2"
                  style={{ color: "rgba(251, 250, 249, 0.7)" }}
                >
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            >
              ...
              <div className="space-y-2 text-xs sm:text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="truncate ml-2" style={{ color: "#FBFAF9" }}>
                    {formatBigInt(
                      userPosition.tokenAAmount,
                      TOKENS.CAMP.decimals,
                      4
                    )}
                  </span>
                </div>
              ...
              <div className="space-y-2 text-xs sm:text-sm">
                ...
                <div className="flex justify-between">
                  <span className="truncate mr-2" style={{ color: "#FBFAF9" }}>
                    {formatBigInt(
                      (userPosition.tokenAAmount * BigInt(removePercentage)) /
                        BigInt(100),
                      TOKENS.CAMP.decimals,
                      4
                    )}{" "}
                ...
                <div className="flex justify-between">
                  <span className="truncate mr-2" style={{ color: "#FBFAF9" }}>
                    {formatBigInt(
                      (userPosition.tokenBAmount * BigInt(removePercentage)) /
                        BigInt(100),
                      TOKENS.USDC.decimals,
                      4
                    )}{" "}
            ...
            <button
              ...
              style={{
                background:
                  isLoading || userPosition.lpTokenBalance === BigInt(0)
                    ? "rgba(239, 68, 68, 0.3)"
