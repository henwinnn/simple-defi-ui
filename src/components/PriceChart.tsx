...
const PriceChart = () => {
  ...
  const fetchHistoricalData = async () => {
    ...
    try {
      ...
      const getBlockRange = () => {
        ...
        switch (timeFrame) {
          case "1H":
            return { blocks: blocksPerHour, points: 12 };
          case "1D":
            return { blocks: blocksPerHour * 24, points: 24 };
          case "1W":
            return { blocks: blocksPerHour * 24 * 7, points: 48 };
          case "1M":
            return { blocks: blocksPerHour * 24 * 30, points: 60 };
          default:
            return { blocks: blocksPerHour * 24, points: 24 };
        }
      };
      ...
      for (const event of swapEvents) {
        ...
        if (timestamp >= oneDayAgo) {
          ...
          const volumeUSD = swapArgs.tokenAtoB
            ? (Number(swapArgs.amountIn) / Math.pow(10, 18)) *
              calculateRealPrice(poolInfo.reserveA, poolInfo.reserveB)
      ...
      for (let i = allEvents.length - 1; i >= 0; i--) {
        ...
        const historicalPrice = calculateRealPrice(
          currentReserveA,
          currentReserveB
        );
        const historicalTVL = calculateTVL(
          currentReserveA,
          currentReserveB,
          historicalPrice
        );
  ...
  const currentRealPrice = calculateRealPrice(
    poolInfo.reserveA,
    poolInfo.reserveB
  );
  const currentTVL = calculateTVL(
      ...
      <div className="glass rounded-2xl p-4 sm:p-6 lg:p-8 border border-white/10 shadow-2xl">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4 sm:mb-6 space-y-4 lg:space-y-0">
          ...
          <div className="flex-shrink-0">
            <div
              className="flex space-x-1 p-1 rounded-lg border w-full sm:w-auto"
              style={{
            ...
            >
              {(["1H", "1D", "1W", "1M"] as const).map((tf) => (
                <button
                  ...
                  style={{
                    color:
                      timeFrame === tf ? "#FBFAF9" : "rgba(251, 250, 249, 0.7)",
                  }}
                >
                  {tf}
                </button>
