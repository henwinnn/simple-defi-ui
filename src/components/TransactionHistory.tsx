...
import { CONTRACTS, SIMPLE_DEX_ABI } from "../constants";
...
const TransactionHistory = () => {
  ...
  const fetchHistoricalTransactions = async (fromBlock?: bigint) => {
    ...
    try {
      ...
      const [swapEvents, liquidityAddedEvents, liquidityRemovedEvents] =
        await Promise.all([
          // Fetch Swap events
      ...
      console.log(
        `Found ${swapEvents.length} swaps, ${liquidityAddedEvents.length} liquidity adds, ${liquidityRemovedEvents.length} liquidity removes`
      );
  ...
  const fetchHistoricalTransactionsCallback = useCallback(
      ...
      logs.forEach(async (log) => {
        if ("args" in log && log.args) {
          try {
            ...
            console.error(
              "Error processing new liquidity removed event:",
              error
            );
          }
        }
            ...
            filteredTransactions.map((tx) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 sm:gap-4 flex-1">
                  <div
                    className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center border"
                    style={{
          ...
          {!isLoading && filteredTransactions.length >= 10 && (
            <div className="text-center pt-4">
              <button
                ...
                style={{
                  background:
                    "linear-gradient(135deg, rgba(131, 110, 249, 0.2), rgba(160, 5, 93, 0.2))",
          ...
          <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-white/10">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              ...
              >
                ...
                >
                  {
                    transactions.filter((tx) => tx.type === "add_liquidity")
                      .length
                  }
                </div>
                <div
              ...
              >
                ...
                >
                  {
                    transactions.filter((tx) => tx.type === "remove_liquidity")
                      .length
                  }
                </div>
                <div
