      ...
      reserveIn > 0 && reserveOut > 0
        ? Math.abs(
            Number(amountOut) /
              ...
              (Number(reserveOut) / Math.pow(10, tokenOut.decimals)) -
              Number(amountInBigInt) /
                Math.pow(10, tokenIn.decimals) /
                (Number(reserveIn) / Math.pow(10, tokenIn.decimals))
          ) * 100
        : 0;
  ...
  const executeSwap = async (swapData: SwapData): Promise<boolean> => {
    ...
    try {
      const amountInBigInt = parseTokenAmount(
        swapData.amountIn,
        swapData.tokenIn.decimals
      );
      const amountOutBigInt = parseTokenAmount(
        swapData.amountOut,
        swapData.tokenOut.decimals
      );
