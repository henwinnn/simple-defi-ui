...
export const useLiquidity = () => {
  ...
  const calculateAddLiquidity = (
    ...
    return {
      ...
      tokenB,
      amountA: (
        Number(optimal.amountA) / Math.pow(10, tokenA.decimals)
      ).toString(),
      amountB: (
        Number(optimal.amountB) / Math.pow(10, tokenB.decimals)
      ).toString(),
  ...
  const executeAddLiquidity = async (
    ...
    try {
      const amountA = parseTokenAmount(
        ...
        liquidityData.tokenA.decimals
      );
      const amountB = parseTokenAmount(
        liquidityData.amountB,
        liquidityData.tokenB.decimals
      );
